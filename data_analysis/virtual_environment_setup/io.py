...
def _write_vmrk_file(vmrk_fname, eeg_fname, events):
    ...
    with codecs.open(vmrk_fname, 'w', encoding='utf-8') as fout:
        ...
        # get rid of comma separation in string
        # mapping[3] = 'Red cross both screens (exp)'
        # END
        ...
        for ii, irow in enumerate(range(len(events)), start=2):
            ...
            print(r'Mk{}=Stimulus,{},{},1,0'
                  .format(ii,tformat.format(i_val)+' '+mapping[i_val], i_ix), file=fout) # CUSTOM CODE by mtiessen --> +' '+mapping[i_val]
